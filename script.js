...
const bgColorsInput = rootElement.querySelectorAll(".settings__back-color")
...
form.addEventListener("change", (event) => {
    ...
    texts.forEach((element) => {
        element.style.lineHeight = settingsIntervalInput.value;
    });
    bgColorsInput.forEach((element) => {
    if (element.checked){
        rootElement.style.backgroundColor = element.value
    }
    });
});
